...
The dataset contains **`r length(activity$steps)`** observations.
...
The mean of the total number of steps taken per day is **`r stepsMean`** steps.

The median of the total number of steps taken per day is **`r stepsMedian`** steps.
...
The interval with the maximum number of steps is the number **`r intervalWithMaxSteps`**
...
The mean of total number of steps taken per day is **`r meanWithoutNA`** and the median is **`r medianWithoutNA`**
...
invisible(Sys.setlocale("LC_TIME", "English")) # this is required to have the names of the day in English
```
